...
local function update_matrices(skeleton, base, pose, indices)
	...
	for i, joint in ipairs(skeleton) do
		local m = identity
		if pose[i] then
			m = cpml.mat4.from_transform(pose[i].translate, pose[i].rotate, pose[i].scale)
		else
			m = cpml.mat4.from_transform(joint.position, joint.rotation, joint.scale)
		end
...
function anim:rebind(data)
	...
	for i, bone in ipairs(data.skeleton) do
		local m = cpml.mat4.from_transform(bone.position, bone.rotation, bone.scale)
