...
class Node<T> {
	...
	final objects: Array<Entry<T>> = [];

	/** Child nodes **/
	final children: Array<Node<T>> = [];
	...
	**/
	inline function set_values(baseLength: Float, minSize: Float, looseness: Float, center: Vec3) {
		...
		final quarter           = this.baseLength / 4;
		final childActualLength = (this.baseLength / 2) * this.looseness;
		final childActualSize   = new Vec3(childActualLength, childActualLength, childActualLength);
		this.childBounds = [
	...
	**/
	...
	public function cast_ray(ray, func: HitFn<T>, depth: Int) {
		if (Intersect.ray_aabb(ray, this.bounds).exists()) {
	...
	**/
	public inline function set_children(childOctrees: Array<Node<T>>) {
		...
		for (i in 0...8) {
			this.children[i] = childOctrees[i];
		}
	}
	...
	**/
	public function shrink_if_possible(minLength: Float): Node<T> {
		...
		for (i in 0...this.objects.length) {
			final object = this.objects[i];
			final newBestFit = this.best_fit_child(object.bounds);
	...
	function split() {
		...
		final quarter   = this.baseLength / 4;
		final newLength = this.baseLength / 2;
...
class Octree<T> {
	...
	final initialSize: Float;
	/** Minimum side length that a node can be - essentially an alternative to having a max depth **/
	final minSize: Float;
	/** Should be a value between 1 and 2. A multiplier for the base size of a node.
	 * 1.0 is a "normal" octree, while values > 1 have overlap **/
	final looseness: Float;
	...
	**/
	public function remove(obj: T) {
		final removed = this.rootNode.remove(obj);
	...
	**/
	function grow(direction: Vec3) {
		...
		if (direction.x >= 0) xDirection = 1;

		var yDirection: Float = -1;
		if (direction.y >= 0) yDirection = 1;

		var zDirection: Float = -1;
		if (direction.z >= 0) zDirection = 1;

		final oldRoot   = this.rootNode;
		final half      = this.rootNode.baseLength / 2;
		final newLength = this.rootNode.baseLength * 2;
		final newCenter = this.rootNode.center + new Vec3(xDirection * half, yDirection * half, zDirection * half);

		// Create a new, bigger octree root node
		this.rootNode = new Node(newLength, this.minSize, this.looseness, newCenter);

		// Create 7 new octree children to go with the old root as children of the new root
		final rootPos  = get_root_pos_index(xDirection, yDirection, zDirection);
		final children = [];
		...
		for (i in 0...8) {
			if (i == rootPos) {
				...
				xDirection = 1;
				if (i % 2 == 0) xDirection = -1;

				yDirection = 1;
				if (i > 3) yDirection = -1;

				zDirection = 1;
				if ((i < 2 || (i > 3 && i < 6))) zDirection = -1;
